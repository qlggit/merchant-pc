$(function(){
    ...
    function doSearch(){
        $.get('/permission/merchant/role/data' ,$searchForm.serialize(), function(a){
            roleData = a.data;
            setPage();
        });
    }
    ...
    function setPermissionList(){
        Promise.all([
            new Promise(function(rev , rej){
                $.get('/permission/merchant/data' , function(a){
                    permissionData = a.data;
                    rev();
                });
            }),
            new Promise(function(rev , rej){
                $.get('/permission/merchant/menu/data' , function(a){
                    menuData = a.data;
                    rev();
                });
            }),
    ...
    $window.find('.this-submit-btn').click(function(){
        ...
        $.post('/permission/merchant/role/' + (updateData?'update':'add') , data , function(a){
            if(a.code ==0){
                $window.modal('hide');
                doSearch();
            }else{
    ...
    $permissionRelationWindow.find('.this-submit-btn').click(function(){
        ...
        $.post('/permission/merchant/role/update' , updateData , function(a){
            if(a.code ==0){
